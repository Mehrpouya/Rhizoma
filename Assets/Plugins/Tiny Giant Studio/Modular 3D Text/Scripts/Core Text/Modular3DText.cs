...
namespace TinyGiantStudio.Text
{
    ...
    public class Modular3DText : MonoBehaviour
    {
            ...
            if (GetComponent<LayoutGroup>())
            {
                if (repositionOldCharacters)
                {
                    ...
                    if (autoFontSize)
                    {
                        if (TryGetComponent<LinearLayoutGroup>(out var linearLayoutGroup))
                        {
                            float multiplier = maxWidth / linearLayoutGroup.totalSpaceTaken;
                        ...
                        else if (TryGetComponent<GridLayoutGroup>(out var gridLayoutGroup))
                        {
        ...
        private void CreateSingleMesh()
        {
            ...
            {
                meshLayouts = linearLayoutGroup.GetPositions(meshLayouts, out float totalSpaceTaken);
                float multiplier = maxWidth / totalSpaceTaken;
                var correctedFontSize = GetChildSize(multiplier);
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            if (linearLayoutGroup.alignment == LinearLayoutGroup.Alignment.Right)
                offset = new Vector3(-maxWidth / 2f, 0, 0);
            else if (linearLayoutGroup.alignment == LinearLayoutGroup.Alignment.Left)
                offset = new Vector3(maxWidth / 2f, 0, 0);

            Gizmos.DrawWireCube(offset, new Vector3(maxWidth, 1, 1));
        }
